...
class newsCrawler:
    ...
    def get_information(self, browser, url):
        ...
        result["content_length"] = self.get_content_length(content["maintext"])
        return result

    def get_page(self, browser, url):
    ...
    def get_content(self, url):
        ...
        content = dict()
        content["date_publish"] = str(article.date_publish)
        content["language"] = article.language
        content["source_domain"] = article.source_domain
        content["maintext"] = article.maintext
        content["title_NP"] = article.title
        return content
    ...
    def asyn_np(self, url_list, results):
        future_to_url = dict()
        for i, url in enumerate(url_list):
            t = self.executor.submit(self.get_newsplease, url=url_list[i])
            future_to_url[t] = url
        for future in concurrent.futures.as_completed(future_to_url):
            url = future_to_url[future]
            try:
    ...
    def asyn_ps(self, url_list):
        future_to_url = dict()
        for i, url in enumerate(url_list):
            t = self.executor.submit(self.get_page,
                                     browser=self.workers[i], url=url_list[i])
            future_to_url[t] = url
        for future in concurrent.futures.as_completed(future_to_url):
            url = future_to_url[future]
            try:
                page = future.result()
                filename = url.replace(':', '%3')
                filename = filename.replace('/', '%2')
                news_path = self.source_path  + 'news/' + filename + '.html'
                with open(news_path, 'w') as out:
                    for line in page:
                        out.write(line+'\n')
                    out.close()
            except Exception as exc:
